...
const normalizeAnswerIndex = (answerIndex) => {
    if (answerIndex === null || answerIndex === undefined || answerIndex === '' || answerIndex === -1) {
        return -1;
    }
...
const ensureCurrentQuestionAnswers = (game) => {
    ...
    game.players.forEach((player) => {
        const existingAnswer = player.answers?.find(a => a.questionIndex === currentQuestionIndex);

        if (existingAnswer) {
            if (isUnansweredAnswer(existingAnswer)) {
                existingAnswer.answerIndex = -1;
                existingAnswer.isCorrect = false;
                existingAnswer.timeTaken = existingAnswer.timeTaken || 0;
...
const submitAnswer = async (req, res) => {
    try {
        ...
        const normalizedAnswerIndex = normalizeAnswerIndex(answerIndex);
        const hasSubmittedAnswer = normalizedAnswerIndex >= 0;
        const isCorrect = hasSubmittedAnswer ? Number(normalizedAnswerIndex) === Number(currentQuestion.correctAnswer) : false;
        const score = hasSubmittedAnswer ? calculateScore(isCorrect, timeTaken, currentQuestion.timeLimit) : 0;
            ...
            },
            {
                $push: {
                    'players.$.answers': {
                        ...
                        isCorrect: hasSubmittedAnswer ? isCorrect : false,
                        timeTaken: timeTaken,
                        score: score
                    }
                },
        ...
        res.status(200).json({
            ...
            message: hasSubmittedAnswer ? (isCorrect ? 'Correct Answer! 🎉' : 'Wrong Answer! ❌') : 'Answer Not Submitted'
        });
...
const endGame = async (req, res) => {
    try {
        ...
        ensureCurrentQuestionAnswers(game);
        await game.save();
